...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(
        ...
        batch_size=1, shuffle=False # batch size 1 for R-peak based TSR logic
    )
    ...
    # Optimal timestep from prior analysis (or a fixed standard like t=50)
    best_t = 50 
    ...
    with torch.no_grad():
        for i, (time_ecg, spectrogram_ecg, r_index) in tqdm(enumerate(test_loader), total=len(labels)):
            ...
            # Apply static mask logic for TSR
    ...
    print(f"Baseline TSR-Net AUC: {auc_tsr:.4f}")
    print(f"Baseline Noise AUC (t={best_t}): {auc_noise:.4f}")
    
    # Evaluate Fusion
    alphas = [0.1, 0.3, 0.5, 0.7, 0.9]
    print("\n--- Fusion Results ---")
    for alpha in alphas:
        fused_scores = alpha * tsr_norm + (1 - alpha) * noise_norm
        auc_fused = roc_auc_score(labels, fused_scores)
        print(f"Alpha={alpha:.1f} (TSR Weight) | AUC: {auc_fused:.4f}")
